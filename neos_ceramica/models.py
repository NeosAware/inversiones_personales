...
class CeramicaHolding(models.Model):
...
class CeramicaAnnualValuation(models.Model):
    ...
    balance_pdf = models.FileField(upload_to="neos_ceramica/annual/%Y", blank=True)
    profit_loss_pdf = models.FileField(upload_to="neos_ceramica/annual/%Y", blank=True)
    corporate_tax_pdf = models.FileField(upload_to="neos_ceramica/annual/%Y", blank=True)
    ...
    def __str__(self):
        return f"Neos Ceramica {self.year}"
