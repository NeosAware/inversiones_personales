...
class MaterialsHolding(models.Model):
    ...
    def as_portfolio_position(self):
        return build_metrics(
            ...
            asset_type="Neos Materials",
            invested_amount=self.invested_amount,
            current_value=self.current_valuation,
            annual_income=self.annual_dividend_income,
            app_url_name="neos_materials:list",
            notes=self.notes,
        )


class MaterialsAnnualValuation(models.Model):
    ...
    balance_pdf = models.FileField(upload_to="neos_materials/annual/%Y", blank=True)
    profit_loss_pdf = models.FileField(upload_to="neos_materials/annual/%Y", blank=True)
    corporate_tax_pdf = models.FileField(upload_to="neos_materials/annual/%Y", blank=True)
    ...
    def __str__(self):
        return f"Neos Materials {self.year}"
